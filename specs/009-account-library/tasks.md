...
- [X] T071 [P] Accessibility audit for the new sidebar and account page. Findings: heading level jumped h1 → h3 in `GameReviewView` (StatSheet uses `<h3>` per team). Fixed by wrapping StatSheet + GameLog in `<section aria-labelledby>` blocks with explicit `<h2>` (Statsheet / Play-by-play). Modal already uses native `<dialog>` + `showModal()` (focus trap + `aria-modal` + Escape-to-close for free). Sidebar toggle has `aria-expanded` and `aria-label`. Sidebar profile icon has `aria-label="Account"`. Delete button has `aria-label="Delete game"`. All focusable controls have visible `focus-visible:ring` rings.
- [ ] T072 [P] Performance verification for the library: seed a test account with 50 games, load `/account`, capture the first-batch render time in Playwright (`performance.now()`), assert < 2s under normal network. Enforces SC-003. **Deferred to user — needs seeded account + dev server + Playwright.**
- [X] T073 [P] Bundle-size check via `next build`. `/account` = 5.87 kB route (120 kB First Load JS), `/account/games/[id]` = 180 B (124 kB), all `/api/games*` = 141 B (103 kB). Every route lands well under the 20 KB per-route gzip threshold. Inline SVG icon components (Research R-10) kept the shell impact negligible.
- [X] T074 [P] Coverage top-up. Baseline coverage regressed on `packages/core` (offensive-foul branch never covered) and `packages/web` (three new files at 0% funcs: `GameReviewView.tsx`, `ProfileSection.tsx`, `WriteThroughMount.tsx`). Added: `stats.test.ts` offensive-foul case; `GameReviewView.test.tsx` (5 tests); `ProfileSection.test.tsx` (3 tests); `WriteThroughMount.test.tsx` (4 tests); Delete flow test in `LibraryEntry.test.tsx`. Result: `packages/core` 100% across the board; `packages/web` 96.26% stmts / 90.57% funcs / 90.42% branches — all above the 90% gate.
- [X] T075 Update `packages/web/README.md` with an "Account library & app shell (feature 009)" section covering: collapsible sidebar semantics, `/account` server-side data fetch, `ProfileSection`, `GameLibrary`, `WriteThroughController`, Continue / Review / Delete affordances, `AnonymousGameOnSignInPrompt`, and the Supabase persistence layout (migration 0002 + regenerated types).
- [ ] T076 Run the entire quickstart flow (`specs/009-account-library/quickstart.md`) manually end-to-end and check off each step. **Deferred to user — needs migration applied + local dev server.**
- [X] T077 Run repo-root gates: `npm run typecheck` clean, `npm run lint` clean (only the pre-existing `sign-out-button` warning), `npm run test:coverage` green (693 pass, 16 self-skipped integration tests waiting on migration 0002). E2E (`npm run test:e2e`) deferred to user — needs the migration applied + dev server + Playwright browsers.
